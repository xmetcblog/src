		...
		<el-row :gutter="20">
			<el-col :span="8">
				...
				<el-card shadow="hover" style="height:252px;">
					...
					<div>
						<ul v-for="(item,ind) in categoryList" :key="ind">
							<li>
								<i>{{item.cateName}}</i>
							</li>
						</ul>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// nickName: windows.sessionStorage.getItem('user'),
		...
		computed: {
			role() {
				return localStorage.getItem('role') == '0' ? '超级管理员' : '普通用户';
			}
		},
		methods: {
			...
			getPageAllArticle:function() {
				axios.get('http://localhost:8763/article/PageAllArticleOrCon')
					.then(response => {
						var articleList = response.data.list;
